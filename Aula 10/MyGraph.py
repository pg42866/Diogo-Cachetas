...
class MyGraph:
    ...
    def check_if_valid_path(self, p): #vê se o path é valido
    ...
    def check_if_hamiltonian_path(self, p): #vê se o caminho é hamiltoniano
    ...
    def search_hamiltonian_path(self): #procura um caminho hamiltoniano para o grafo, a partir de todos os nodes
    ...
    def search_hamiltonian_path_from_node(self, start): #procura um caminho hamiltoniano, dado um node inicial (start)
        current = start
        visited = {start:0} #nodes visitados
        path = [start] #caminho
        while len(path) < len(self.get_nodes()): #enquanto os nodes não tiverem sido todos corridos
            nxt_index = visited[current] #proximo indice vai ser o numero do node corrente
            if len(self.graph[current]) > nxt_index: #se o tamanho das ligações for maior do que o indice anterior
                nxtnode = self.graph[current][nxt_index] 
                visited[current] += 1 #incrementa-se os vsitados em 1
                if nxtnode not in path: #se o proximo node ainda não estiver no caminho
                    path.append(nxtnode) #adiciona-se ao caminho
                    visited[nxtnode] = 0                    
                    current = nxtnode #mudamos o corrent node para este     
            else: 
                if len(path) > 1: #se o comprimento do caminho for maior do que 1
                    rmvnode = path.pop() #remove o 
                    del visited[rmvnode]
                    current = path[-1]
                else:
                    return None
    ...
    def check_balanced_node(self, node): #vê se o node é balanced, ou seja se tem o mesmo numero de entradas e saidas
        return self.in_degree(node) == self.out_degree(node)
        
    def check_balanced_graph(self): #vê se o grafo é balanced
        for n in self.graph.keys():
            if not self.check_balanced_node(n):
                return False
        return True
    
    def check_nearly_balanced_graph(self): #ve se o grafo é quase balanced, ou seja, apenas 2 nodes desse grafo podem ter diferença entre entrada e saída, sendo essas diferenças de 1 e -1
        res = None, None
        for n in self.graph.keys():
    ...
    def is_connected(self): #vê se todos os nodes do grafo estão ligados
    ...
    def eulerian_cycle(self):
        if not self.is_connected() or not self.check_balanced_graph(): # se o grafo não for connected ou balanceado
            return None
        arcos_visitar = list(self.get_edges()) #arcos a visitar
        res = []
        while arcos_visitar: #enquanto houverem arcos para visitar
            par = arcos_visitar[0] 
            i = 1
            if res != []: #se a lista final não for vazia
                while par[0] not in res: #enquanto tuplo não estiver na lista
                    par = arcos_visitar[i] #pair toma os valores do tuplo seguinte
                    i += 1 #incrementa-se em 1 o valor de i
            arcos_visitar.remove(par) #remove-se essa lista de tuplos dos arcos a visitar
            start, nxt = par # variaveis que vão guardar o no de origem e o destino
            ciclo = [start, nxt] #dar os valores iniciais ao ciclo
            while start != nxt: #enquanto o nodo de inicio e nodo seguinte forem diferentes
                for suc in self.graph[nxt]: #para cada sucessor do nodo next
                    if (nxt, suc) in arcos_visitar: #se o next e o sucessor estiverem nos arcos a ser visitados
                        par = (nxt, suc) #pair passa a tomar os valores de nxt e sucessor
                        nxt = suc #o sucessor passa a ser chamado de next
                        ciclo.append(nxt) #e dá-se append ao ciclo
                        arcos_visitar.remove(par) #remove-se esse arco da lista de arcos a visitar
            if not res: #se res estiver vazia
                res = ciclo #a lista final passa a ser o ciclo
            else: #se res não estiver vazia
                pos = res.index(ciclo[0]) #procura o indice correspondente ao valor que tem no primeiro elemento do ciclo
                for i in range (len(ciclo)-1): #para cada posição no ciclo
                    res.insert(pos + i + 1, ciclo[i+1]) #insere-se 
        return res                 
      
    def eulerian_path(self): #path euleriano para um graph quase balanceado
